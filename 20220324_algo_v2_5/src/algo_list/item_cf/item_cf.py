...
class ItemCF(object):
    ...
    def __init__(self, usr, database, train_data=None, test_flag=False):
        ...
        self.__like_users = self.__database.like_users
        self.__liked_dynamics = self.__database.liked_dynamics
        self.__like_users_map = self._map_list_to_index(self.__like_users)
        self.__liked_dynamics_map = self._map_list_to_index(self.__liked_dynamics)
        self.__like_users_reverse_map = self._map_index_to_list(self.__like_users)
        self.__liked_dynamics_reverse_map = self._map_index_to_list(self.__liked_dynamics)
    ...
    def _create_ui_matrix(self) -> np.ndarray:
        ui_matrix = np.zeros((len(self.__like_users), len(self.__liked_dynamics)), dtype="float32")
        for u in self.__like_users:
            if self.__test_flag:
                like_items = self.__train_data[str(u)]
            else:
                like_items = self.__database.get_objs(["user", u, 'like', 'item'], key="动态")
            # like_items_map = [dynamics_map[s] for s in like_items]
            for i in like_items:
                ui_matrix[self.__like_users_map[u]][self.__liked_dynamics_map[i]] = 1
    ...
    def get_top_n(self) -> list:
        ...
        def _get_item_list(row, reverse_map, top_n=45):
            ...
            if self.__test_flag:
                like_items = self.__train_data[str(row["user"])]
            else:
                like_items = self.__data_base.get_objs(["user", str(row["user"]), 'like', 'item'], key="动态")
            view_items = self.__data_base.get_objs(['user', str(row["user"]), 'view', 'item'], key="动态")
            create_items = self.__data_base.get_objs(['user', str(row["user"]), 'create', 'item'], key="动态")
            items = [i for i in items if (i not in like_items) and (i not in view_items) and (i not in create_items)]
            return items[: top_n]

        ui_matrix = self._create_ui_matrix()
        item_similarities_ = self._calc_item_similarities(ui_matrix)
        pred = self._predict(ui_matrix, item_similarities_)

        sorted_items = np.argsort(-pred)  # 根据pred，对每个用户的物品进行降序排序
        df = pd.DataFrame(data=sorted_items)
        df["user"] = np.arange(len(df))
        df["user"] = df.apply(lambda x: self.__like_users_reverse_map[x["user"]], axis=1)  # 根据行索引增加用户栏
        cols = df.columns
        top_n = 45
        df["top_n"] = df.apply(lambda x: _get_item_list(x[cols], self.__dynamics_reverse_map, top_n),
                               axis=1)  # 获得每位用户的top45物品列表
    ...
    def run_item_cf(self, user_id, data_base_, train_data=None, test_flag=False):
        """
        对user_id用户进行物品推荐，该过程读取.feather文档，找到用户对应的前N名物品随后增加最受欢迎的物品
        :return:
        """
        top_n_df = feather.read_dataframe(
            os.path.join(configs.rec_result_folder_path, "item_cf_top_n_recommendation_map.feather"))
        if top_n := list(top_n_df[top_n_df["user"] == str(self.__usr)]["top_n"]):
