...
from algos import GeneralizedCF, ItemCF
...
class AlgosOperator(object):
    ...
    def run_all_algos(self) -> list:
        ...
        """
        algo_res_list = []
        for algo_name in ['generalized_cf', 'item_cf']:
    ...
    def run_all_users(self, algo_name, item_cat="动态"):
        ...
        if item_cat == "动态":
            ...
            if algo_name == "generalized_cf":
                for usr in self._database.like_users:
                    generalized_cf = GeneralizedCF(usr, self._database)
                    gama = generalized_cf.run_generalized_cf()
                    gama_sets.append(",".join(gama))

            elif algo_name == "item_cf":
                item_cf = ItemCF(self._database)
                item_cf.get_top_n()

                for usr in self._database.like_users:
                    gama = item_cf.run_item_cf(usr)
                    gama_sets.append(",".join(gama))

            gama_df = pd.DataFrame(data={"user": self._database.like_users, "recommendation": gama_sets})
    ...
    def train_test_split(self, train_ratio=0.5):
        for usr in self._database.like_users:
            like_items = self._database.get_objs(['user', usr, 'like', 'item'], key="动态")
    ...
    def get_recommendation(self, algo_name, item_category="动态"):
        if item_category == "动态":
            ...
            if algo_name == "generalized_cf":
                for usr in self._database.like_users:
                    generalized_cf = GeneralizedCF(usr, self._database, train_data=train_data, test_flag=True)
                    gama = generalized_cf.run_generalized_cf()
                    for recommend_item in gama:
                        recommend_data.append([usr, recommend_item])

            elif algo_name == "item_cf":
                item_cf_ = ItemCF(self._database, train_data, True)
                item_cf_.get_top_n()  # 此处有错

                for usr in self._database.like_users:
                    gama = item_cf_.run_item_cf(usr)
                    for recommend_item in gama:
                        recommend_data.append([usr, recommend_item])
...
if __name__ == "__main__":
    ...
    # algos.run_all_algos()
    algos.calc_and_store_metrics(5)
