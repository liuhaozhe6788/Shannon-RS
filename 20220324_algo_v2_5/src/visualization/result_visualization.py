...
def accu_vis(algo_name_, gama_result, data_base_):
    """
    所有CLUB标签的推荐精度计算与可视化
    一个CLUB标签c的推荐精度的计算：
    ...
    my_plot.bar_plot(x="x_values",
                     ...
                     new_legend_labels=[f'算法{algo_name_}的推荐性能'],
                     new_xlabel="CLUB标签",
                     new_ylabel="精度",
                     new_title=f"算法{algo_name_}的推荐性能",
                     new_fig_name=os.path.join(configs.perf_result_folder_path, f"{algo_name_}_accu_results.png"),
                     savefig=True
                     )
