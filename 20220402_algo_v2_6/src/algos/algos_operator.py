...
from .algos_list import GeneralizedCF, ItemCF, UserCF, Basic
...
class AlgosOperator(object):
    ...
    def run_all_algos(self) -> list:
        ...
        """
        algo_res_list = []
        for algo_name in ['generalized_cf', 'item_cf', 'user_cf']:
    ...
    def get_recommendation(self, algo_name, item_category="动态"):
        if item_category == "动态":
            ...
            if algo_name == "generalized_cf":
                for usr in self.database.like_users:
                    generalized_cf = GeneralizedCF(usr, self.database, train_data=train_data, test_flag=True)
                    gama = generalized_cf.run_generalized_cf()
                    for recommend_item in gama:
                        recommend_data.append([usr, recommend_item])

            elif algo_name == "item_cf":
                item_cf_ = ItemCF(self.database, train_data, True)
                ...
                user_cf_ = UserCF(self.database, train_data, True)
                user_cf_.get_top_n()  # 此处有错
