...
def accu_vis(algo_names, gama_results, data_base_):
    """
    所有CLUB标签的推荐精度计算与可视化
    一个CLUB标签c的推荐精度的计算：
    ...
    for algo in algo_names:
        ...
        accuracies = np.zeros(len(like_occurence), dtype=float)
        np.divide(accur_occurence, like_occurence, accuracies)
        all_algos_accuracies.append(accuracies)
    ...
    df = pd.DataFrame({
        ...
        "cat": [item for item in algo_names for _ in range(len(accuracies))]
    })
    ...
    my_plot.bar_plot(x="x_values",
                     ...
                     new_legend_labels=[f'算法{i}的推荐性能' for i in algo_names],
                     new_xlabel="CLUB标签",
                     new_ylabel="精度",
                     new_title=f"所有推荐算法的推荐性能",
                     new_fig_name=os.path.join(configs.perf_result_folder_path, f"accu_results.png"),
                     savefig=True
                     )
    return None
...
if __name__ == "__main__":
    ...
    algo_names = ["generalized_cf", "item_cf", "user_cf"]
    gama_dfs = []
    for algo_name in algo_names:
        gama_df = myalgoOperator.run_all_users(algo_name)
        club_dist_vis(algo_name, gama_df, mydb)
        gama_dfs.append(gama_df)
    gama_results = dict(zip(algo_names, gama_dfs))
    accu_vis(algo_names, gama_results, mydb)
