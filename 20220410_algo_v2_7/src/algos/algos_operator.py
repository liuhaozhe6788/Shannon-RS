...
class AlgosOperator(object):
    ...
    def get_recommendation(self, algo_name, item_category="动态"):
        if item_category == "动态":
            ...
            if algo_name == "generalized_cf":
                for usr in self.database.like_users:
                    generalized_cf = GeneralizedCF(self.database, train_data, True)
                    gama = generalized_cf.run(usr)
                    for recommend_item in gama:
                        recommend_data.append([usr, recommend_item])

            elif algo_name == "item_cf":
                item_cf_ = ItemCF(self.database, "item_cf_top_n_recommendation_map.feather", train_data, True)
                item_cf_.get_top_n()
                ...
                user_cf_ = UserCF(self.database, "user_cf_top_n_recommendation_map.feather", train_data, True)
                user_cf_.get_top_n()
                ...
                hybrid_cf = HybridCF(self.database, "hybrid_cf_top_n_recommendation_map.feather", train_data, True)
                hybrid_cf.get_top_n()
                for usr in self.database.like_users:
                    gama = hybrid_cf.run(usr)
                    for recommend_item in gama:
