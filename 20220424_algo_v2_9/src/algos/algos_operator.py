...
class AlgosOperator(object):
    ...
    def get_recommendation(self, algo_name):
        ...
        if algo_name == "generalized_cf":
            ...
            item_cf_ = ItemCF(self.database, "item_cf_top_n_recommendation_map.feather", sim_thres=self.item_cf_params[0],
                              pred_thres=self.item_cf_params[1], train_data=train_data, test_flag=True)
            item_cf_.get_top_n()
            ...
            user_cf_ = UserCF(self.database, "user_cf_top_n_recommendation_map.feather", sim_thres=self.user_cf_params[0],
                              pred_thres=self.user_cf_params[1], train_data=train_data, test_flag=True)
            user_cf_.get_top_n()
