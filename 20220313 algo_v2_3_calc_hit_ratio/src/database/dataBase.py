...
class DataBase:
    def __init__(self, data_file_path=None):
        self.__data = {}
        self.__item_cats = check.item_categories
    ...
    def get_clubs(self, key=None, del_prefix: bool = False):
        clubs = self.__clubs
...
#    ic(set(map(lambda club: club.split(":")[-1].split("_")[0], list(filter(lambda s: s.startswith("动态"), data_base.clubs)))))
    ...
    ic(data_base.get_objs(['item', '动态:10355', 'have', 'club']))
#    ic(data_base.clubs)
    ...
    # m = data_base.get_objs(["club", "动态:二级标签:推广集", "have", "selected_item"], key="动态")
    # ic(m)
