...
class AlgosOperator(object):
    ...
    def userCF_tuning(self, sim_thres_range, pred_thres_range):
        ...
        np.save(os.path.join(configs.perf_result_folder_path, "user_cf_tuning.npy"), f_scores)
        np.save(os.path.join(configs.perf_result_folder_path, "user_cf_params.npy"), np.asarray(params))
    ...
    def itemCF_tuning(self, sim_thres_range, pred_thres_range):
        ...
        param_indices = list(zip(result[0], result[1]))[0]
        params = [sim_thres_range[param_indices[0]], pred_thres_range[param_indices[1]]]
        np.save(os.path.join(configs.perf_result_folder_path, "item_cf_tuning.npy"), f_scores)
        np.save(os.path.join(configs.perf_result_folder_path, "item_cf_params.npy"), np.asarray(params))
        return params, f_scores
