...
def _prefilter(user_id: str, train_data: list, data_base: DataBase, test_flag: bool):
    ...
    clubs = set(list(map(lambda x: ":".join(x.split(":")[:-1]), data_base.get_objs(['user', user_id, 'join', 'club'], key="动态"))))  # 所属的Club集
    ...
    # 取集合CLUB的补集的所有内容精选
    e_club_ = set(_choose_contents(user_id, 'club', 'have', 'selected_item', set(list(map(lambda x: ":".join(x.split(":")[:-1]),data_base.clubs))) - clubs, data_base))
...
def _rearrangement(user_id: str, beta: list, n_beta: int, train_data: list, data_base_: DataBase, test_flag: bool):
    ...
    if n_beta == thresholds[0]:
        ...
        [gama, _] = _join_contents_list(m, n_m, beta, n_beta, thresholds[2])
    else:
...
if __name__ == "__main__":
    ...
    print(run_generalized_cf("49070", db))
